...
todo_route_blueprint = Blueprint("routes", __name__)
todo_schema = Todo()

##Create new ToDo
@todo_route_blueprint.post("/api/todo/create")
def create_todo():
...
@todo_route_blueprint.get("/api/todos")
def get_all_todos():
    result = all_todos()
    if result.is_err():
        return jsonify({"error": f"{result.unwrap_err()._message}"}), 400
    data = [todo.to_json() for todo in result.unwrap()]
    return jsonify({"data": data}), 200
    
    
##Edit todo
@todo_route_blueprint.patch("/api/todo/update/<int:id>")
def edit_todo(id):
    req_body = request.json
    req_body["id"] = id
...
##Delete Todo
@todo_route_blueprint.delete("/api/todo/delete/<int:id>")
def delete_todo(id):
